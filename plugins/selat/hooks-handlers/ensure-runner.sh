...
# plugin-owned dir, NOT a selat-cli home — verified against selat-cli@0.15.18, selat-cli
...
# --- validate the runtime home before it reaches a generated script / rc file ---
# SH_HOME comes from the SELAT_PLUGINS_HOME override and gets embedded into the
# generated runner shim (runs on every `selat` call) and into the PATH line appended
# to the user's shell rc (runs at every shell startup). A value containing shell
# metacharacters would break out of the surrounding quoting in those sinks — arbitrary
# code execution. A real runtime path never needs them, so reject them and FAIL CLOSED.
# (Spaces and ordinary path punctuation are allowed; the sinks single-quote the value.)
case "$SH_HOME" in
  *[\"\'\`\$\;\&\|\<\>\(\)\{\}\\!*?]* | *"#"* | *$'\n'* | *$'\r'* | *$'\t'* )
    fail "SELAT runner unavailable: SELAT_PLUGINS_HOME contains characters that are not allowed in a runtime path (quotes, \$, \`, ;, &, |, redirects, braces, backslash, glob, or control chars). Unset it or point it at a plain filesystem path, then retry. SELAT will not embed an untrusted path into a shell startup file."
    ;;
esac

# Single-quote a validated value for safe embedding in generated sh (the value cannot
# contain a single quote — rejected above — so this never breaks). Handles spaces too.
sq() { printf "'%s'" "$1"; }
...
#
# Each embedded path is single-quoted (sq) so its bytes stay a single literal token in
# the generated sh — defense-in-depth over the SH_HOME validation above, so no path can
# inject a command into the shim even if a future change loosens the input rules. \$PATH
# and \$@ are escaped to stay literal (expanded when the shim runs, not now).
SHIM_NPM_CACHE="$(sq "$NPM_CACHE")"
SHIM_NODE_DIR="$(sq "$NODE_BIN_DIR")"
SHIM_NODE_BIN="$(sq "$NODE_BIN")"
SHIM_CLI_ENTRY="$(sq "$CLI_ENTRY")"
cat >"$SHIM_PATH" <<SHIM
#!/usr/bin/env sh
...
# --- put `selat` on PATH ---
# SAFE, always-on mechanisms first (neither edits a shell startup file):
#   1. the session env file (CLAUDE_ENV_FILE) — a per-session export, gone next session;
#   2. a single symlink of the shim into a known per-harness on-PATH bin dir.
# The PERSISTENT edit of ~/.zshrc / ~/.bash_profile / ~/.bashrc is OPT-IN
# (SELAT_PATH_AUTOADD=1). Auto-appending to a user's shell startup file is a surprising,
# durable change (and a persistence footprint), so by default we DON'T — we disclose the
# one-line PATH export and let the user add it, or they use the absolute $SELAT_RUNNER.
if [ -n "${CLAUDE_ENV_FILE:-}" ]; then
  persist_env PATH "$BIN_DIR:$PATH"
fi

# Symlink the shim into known harness bin dirs already on PATH — persists on disk but is
# a single symlink, not an edit to a startup file. Some harnesses don't persist the hook
# env (no CLAUDE_ENV_FILE) AND run agent commands in a shell that doesn't source the rc,
# so without this bare `selat` wouldn't resolve there.
#   OpenClaw: ~/.openclaw/bin — verified on a live OpenClaw session (that dir is on PATH;
#             a `selat` symlink there resolves). Guarded on ~/.openclaw existing so this
#             is a no-op on every other host.
...
# Build the one-line PATH export (used whether we write it or just disclose it). SH_HOME
# was validated above, so $BIN_DIR carries no shell metacharacters — this line is safe to
# embed in an rc file. The $HOME form keeps the literal `$HOME` so it stays portable.
...
if [ "${SELAT_PATH_AUTOADD:-0}" = "1" ]; then
  add_path_to_rc() {
    local rc="$1"
  ...
  log "not editing shell startup files (opt in with SELAT_PATH_AUTOADD=1). For bare 'selat' in new shells add: $PATH_LINE  — or use the absolute runner \$SELAT_RUNNER ($SHIM_PATH)."
fi
...
#
# Verified against @selat-ai/selat-cli@0.15.18 (lib/commands/doctor.mjs + bin/selat.mjs):
# `doctor()` counts hard failures and `return 0` only when failures === 0, else
# `return 1`; main() sets `process.exitCode` from it, so the exit code DOES
